...
VALUES
  ...
  -- 2/ Ecrire une requête pour lister tous les détails(name, manufacturer) de tous les
--téléphone dont le prix est supérieur à 200€
  SELECT * FROM telephones WHERE price > 200;
  
  -- 3/ Ecrire une requête pour lister les détails(name, manufacturer) de tous les
--téléphone dont le prix varie entre 150 et 200€
  SELECT * FROM telephones WHERE price > 150 AND price < 200;
  SELECT * FROM telephones WHERE price BETWEEN 150 AND 200;
  
  -- 4/ Lister tous les téléphones de marque samsung et Apple
  SELECT * FROM telephones WHERE manufacturer LIKE 'Samsung' OR manufacturer LIKE 'Apple';
  SELECT * FROM telephones WHERE manufacturer ILIKE '%Samsung%' OR manufacturer ILIKE '%Apple%';
   
  -- 5/ Afficher le revenu total pour les téléphones vendues.Pour chaque téléphone, vous
--avez le prix et la quantité vendu
  SELECT CONCAT(manufacturer, ' ', name) AS telephone,
  price * units_sold AS revenu_total FROM telephones;
  
  -- 6/ Afficher le nombre des téléphones Apple
  SELECT COUNT(*) FROM telephones WHERE manufacturer LIKE 'Apple';
  SELECT SUM(units_sold) FROM telephones WHERE manufacturer LIKE 'Apple';
  
  -- 7/ Afficher toutes les données de la table téléphone en renommant les champs de la 
 ...
 -- 8/ Afficher tous les téléphones dont le prix total des ventes est supérieur à 1300000.
  SELECT 
  	name AS telephone, 
  	price * units_sold AS revenu_total
  FROM telephones 
  ...
  SELECT manufacturer, name, SUM(units_sold) FROM telephones GROUP BY manufacturer, name ORDER BY manufacturer;
  
  -- groups with units solds greater than 1000
  SELECT 
  	manufacturer, 
	SUM(units_sold), 
	COUNT(id) 
